...
sfdx-dependabot:
  ...
  script:
    - sf simply cicd sfdx-dependabot
      ...
      --change-request-labels dependencies
      --fail-on-error
```

`--vcs-token` (or `SIMPLY_CICD_VCS_TOKEN`, or `SFDX_DEPENDABOT_VCS_TOKEN`) needs file-writing and change-request privileges across every downstream project it might touch — this is necessarily a broader-scoped token than a single project's `CI_JOB_TOKEN`, since the whole point is acting across repositories the triggering pipeline doesn't own.
