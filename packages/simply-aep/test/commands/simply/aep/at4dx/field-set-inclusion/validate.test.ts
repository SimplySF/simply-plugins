...
import At4dxFieldSetInclusionValidate from '../../../../../../src/commands/simply/aep/at4dx/field-set-inclusion/validate.js';

describe('simply aep at4dx field-set-inclusion validate', () => {
  ...
  beforeEach(() => {
    ...
    sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'simply-aep-fsi-validate-'));
  });
  ...
  it('should error when neither --target-org nor --source-dir is specified', async () => {
    try {
      await At4dxFieldSetInclusionValidate.run([]);
  ...
  it('should error when both --target-org and --source-dir are specified', async () => {
    try {
      await At4dxFieldSetInclusionValidate.run(['--target-org', testOrg.username, '--source-dir', sourceDir]);
      expect.fail('should have thrown Error');
    } catch (err) {
      expect((err as SfError).message).to.include('You must specify either --target-org or --source-dir');
    }
  ...
  it('should error when the Custom Metadata Type does not exist in the target org', async () => {
    ...
    try {
      await At4dxFieldSetInclusionValidate.run(['--target-org', testOrg.username]);
      expect.fail('should have thrown Error');
    } catch (err) {
  ...
  it('should error when local source has no matching CustomMetadata components', async () => {
    try {
      await At4dxFieldSetInclusionValidate.run(['--source-dir', sourceDir]);
      expect.fail('should have thrown Error');
    } catch (err) {
      expect((err as SfError).message).to.include("AT4DX doesn't appear to be present");
    }
  ...
  it('should leave process.exitCode unset and return no issues for a well-formed record', async () => {
    ...
    const result = await At4dxFieldSetInclusionValidate.run(['--source-dir', sourceDir, '--json']);
  ...
  it('should set process.exitCode to 1 and still return the full issues array when an error-severity issue is found', async () => {
    writeCustomMetadata(
      ...
      [
        '  <values><field>BindingSObject__c</field><value xsi:type="xsd:string">Account</value></values>',
        '  <values><field>FieldsetName__c</field><value xsi:type="xsd:string">SharedFields</value></values>',
      ].join('\n'),
    );
    writeCustomMetadata(
      'SelectorConfig_FieldSetInclusion.Contact_Fields.md-meta.xml',
      [
    ...
    expect(result.issues.filter((issue) => issue.rule === 'duplicate-fieldset-name')).to.have.lengthOf(2);
  ...
  it('should surface a record with no resolvable SObject as a missing-sobject-reference error', async () => {
    writeCustomMetadata(
      ...
      [
        '  <values><field>BindingSObject__c</field><value xsi:nil="true"/></values>',
        '  <values><field>BindingSObjectAlternate__c</field><value xsi:nil="true"/></values>',
        '  <values><field>FieldsetName__c</field><value xsi:type="xsd:string">SomeFields</value></values>',
      ].join('\n'),
    );
