 ...
 */
export function toDomainProcessBindingCliError(
  error: unknown,
  messages: Messages<string>,
  printIssues: (issues: DomainProcessBindingIssue[]) => void,
): Error {
  if (!(error instanceof DomainProcessBindingWriteError)) {
  ...
  switch (error.code) {
    ...
    case 'context-field-mismatch':
      return messages.createError('error.contextFieldMismatch', [error.message]);
    ...
    case 'at4dx-not-detected':
      return messages.createError('error.at4dxNotDetected');
