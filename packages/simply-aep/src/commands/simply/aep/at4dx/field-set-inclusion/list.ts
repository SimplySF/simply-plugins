/*
 * Copyright (c) 2026, Clay Chipps.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import { Messages } from '@salesforce/core';
import { SfCommand, Flags } from '@salesforce/sf-plugins-core';
import {
  scanLocalFieldSetInclusions,
  scanOrgFieldSetInclusions,
  type At4dxFieldSetInclusionListResult,
  type RawFieldSetInclusionRecord,
} from '@simplysf/simply-aep-core';

Messages.importMessagesDirectoryFromMetaUrl(import.meta.url);
const messages = Messages.loadMessages('@simplysf/simply-aep', 'simply.aep.at4dx.field-set-inclusion.list');

/** The table's presentational shape — `--json` keeps the full `RawFieldSetInclusionRecord` shape; this is display-only. */
type DisplayRow = {
  developerName: string;
  sobject: string;
  fieldsetName: string;
  active: string;
  source: string;
};

function toDisplayRow(record: RawFieldSetInclusionRecord): DisplayRow {
  return {
    developerName: record.developerName,
    sobject: record.sobject,
    fieldsetName: record.fieldsetName,
    active: String(record.isActive),
    source: record.source,
  };
}

/**
 * Lists the AT4DX Selector field set inclusions (`SelectorConfig_FieldSetInclusion__mdt`) configured
 * in a target org or local DX source. Unlike `binding list`, there's no priority/winner concept — every
 * active record contributes its field set to its SObject's selector.
 */
export default class At4dxFieldSetInclusionList extends SfCommand<At4dxFieldSetInclusionListResult> {
  public static readonly summary = messages.getMessage('summary');
  public static readonly description = messages.getMessage('description');
  public static readonly examples = messages.getMessages('examples');

  public static readonly flags = {
    ...SfCommand.baseFlags,
    'target-org': Flags.optionalOrg({
      summary: messages.getMessage('flags.target-org.summary'),
      char: 'o',
    }),
    'api-version': Flags.orgApiVersion(),
    'source-dir': Flags.directory({
      summary: messages.getMessage('flags.source-dir.summary'),
      char: 'd',
      exists: true,
      multiple: true,
    }),
  };

  /** @returns The field set inclusion records found for the requested source. */
  public async run(): Promise<At4dxFieldSetInclusionListResult> {
    const { flags } = await this.parse(At4dxFieldSetInclusionList);

    const targetOrg = flags['target-org'];
    const sourceDirs = flags['source-dir'] ?? [];

    if ((targetOrg && sourceDirs.length > 0) || (!targetOrg && sourceDirs.length === 0)) {
      throw messages.createError('error.targetOrgOrSourceDirRequired');
    }

    let source: string;
    let records: RawFieldSetInclusionRecord[];

    if (targetOrg) {
      const connection = targetOrg.getConnection(flags['api-version']);
      source = targetOrg.getUsername() ?? 'org';

      this.spinner.start(messages.getMessage('info.queryingOrg', [source]));
      let scanResult: Awaited<ReturnType<typeof scanOrgFieldSetInclusions>>;
      try {
        scanResult = await scanOrgFieldSetInclusions(connection);
      } catch (error) {
        this.spinner.stop();
        throw messages.createError('error.orgQueryFailed', [(error as Error).message]);
      }
      this.spinner.stop();

      if (scanResult.missing) {
        throw messages.createError('error.at4dxNotDetected');
      }

      records = scanResult.records;
    } else {
      source = 'local';

      this.spinner.start(messages.getMessage('info.scanningLocalSource'));
      try {
        ({ records } = scanLocalFieldSetInclusions(sourceDirs));
      } catch (error) {
        this.spinner.stop();
        throw messages.createError('error.localScanFailed', [(error as Error).message]);
      }
      this.spinner.stop();

      if (records.length === 0) {
        throw messages.createError('error.at4dxNotDetected');
      }
    }

    this.table({
      data: records.map(toDisplayRow),
      columns: [
        { key: 'developerName', name: 'DEVELOPER NAME' },
        { key: 'sobject', name: 'SOBJECT' },
        { key: 'fieldsetName', name: 'FIELDSET NAME' },
        { key: 'active', name: 'ACTIVE' },
        { key: 'source', name: 'SOURCE' },
      ],
    });

    this.info(messages.getMessage('info.complete', [records.length, source]));

    return { source, records };
  }
}
