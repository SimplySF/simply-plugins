...
import DeployProjectDeployUnpackaged from '../../../../../../src/commands/simply/cicd/deploy/project/deploy-unpackaged.js';

vi.mock('../../../../../../src/common/deploy/deployProject.js', () => ({ deployProject: vi.fn() }));

describe('deploy project deploy-unpackaged', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('requires --ci-job-token', async () => {
    await expect(DeployProjectDeployUnpackaged.run([])).rejects.toThrow(/ci-job-token/i);
  });
