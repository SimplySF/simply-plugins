...
import DeployProjectPreDestructive from '../../../../../../src/commands/simply/cicd/deploy/project/pre-destructive.js';

vi.mock('../../../../../../src/common/deploy/deployProject.js', () => ({ deployProject: vi.fn() }));

describe('deploy project pre-destructive', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('requires --ci-job-token', async () => {
    await expect(DeployProjectPreDestructive.run([])).rejects.toThrow(/ci-job-token/i);
  });

  it('wires flags through to deployProject with the correct stage', async () => {
    vi.mocked(deployProject).mockResolvedValue(undefined);

    await DeployProjectPreDestructive.run([
      '--ci-job-token',
      'tok',
      '--alias',
      'my-org',
