...
import At4dxPlatformEventSubscriptionList from '../../../../../../src/commands/simply/aep/at4dx/platform-event-subscription/list.js';

/* eslint-disable camelcase -- AT4DX Custom Metadata field API names (EventBus__c, Consumer__c, etc.) */

describe('simply aep at4dx platform-event-subscription list', () => {
  ...
  it('should error when neither --target-org nor --source-dir is specified', async () => {
    try {
      await At4dxPlatformEventSubscriptionList.run([]);
  ...
  it('should error when both --target-org and --source-dir are specified', async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'simply-aep-pes-list-'));
    try {
      await At4dxPlatformEventSubscriptionList.run(['--target-org', testOrg.username, '--source-dir', directory]);
      expect.fail('should have thrown Error');
    } catch (err) {
      expect((err as SfError).message).to.include('You must specify either --target-org or --source-dir');
    } finally {
  ...
  it('should error when the Custom Metadata Type does not exist in the target org', async () => {
    ...
    try {
      await At4dxPlatformEventSubscriptionList.run(['--target-org', testOrg.username]);
      expect.fail('should have thrown Error');
    } catch (err) {
  ...
  it('should list platform event subscriptions queried from an org', async () => {
    ...
    const result = await At4dxPlatformEventSubscriptionList.run(['--target-org', testOrg.username, '--json']);
  ...
  it('should list platform event subscriptions scanned from local source', async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'simply-aep-pes-list-local-'));
    try {
      const customMetadataDir = path.join(sourceDir, 'customMetadata');
      fs.mkdirSync(customMetadataDir, { recursive: true });
      fs.writeFileSync(
        path.join(customMetadataDir, 'PlatformEvents_Subscription.Account_Change_Subscriber.md-meta.xml'),
        [
      ...
      const result = await At4dxPlatformEventSubscriptionList.run(['--source-dir', sourceDir, '--json']);

      expect(result.source).to.equal('local');
      expect(result.records).to.have.lengthOf(1);
      expect(result.records[0].consumer).to.equal('AccountChangeConsumer');
    } finally {
  ...
  it('should error when local source has no matching CustomMetadata components', async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'simply-aep-pes-list-empty-'));
    try {
      await At4dxPlatformEventSubscriptionList.run(['--source-dir', sourceDir]);
      expect.fail('should have thrown Error');
    } catch (err) {
      expect((err as SfError).message).to.include("AT4DX doesn't appear to be present");
    } finally {
