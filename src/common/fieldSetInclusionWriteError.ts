...
import { FieldSetInclusionWriteError, type FieldSetInclusionIssue } from '@simplysf/simply-aep-core';

/**
 * Maps a `FieldSetInclusionWriteError` thrown by `createFieldSetInclusion`/`updateFieldSetInclusion`
 * onto the calling command's own `error.*` message keys, shared by `create.ts`/`update.ts` since both
 * throw the same error type for the same set of codes. Mirrors `bindingWriteError.ts`'s
 * `toBindingCliError` — see docs/design/0016-at4dx-selector-config-field-set-inclusion.md.
 *
 * @param error - Whatever `createFieldSetInclusion`/`updateFieldSetInclusion` rejected with.
 ...
 * @returns An `Error` ready to throw from the command's `run()` — an `SfError` built from a message key when `error` is a recognized `FieldSetInclusionWriteError`, or `error` itself (coerced to `Error`) otherwise.
 */
export function toFieldSetInclusionCliError(
  error: unknown,
  messages: Messages<string>,
  printIssues: (issues: FieldSetInclusionIssue[]) => void,
): Error {
  if (!(error instanceof FieldSetInclusionWriteError)) {
