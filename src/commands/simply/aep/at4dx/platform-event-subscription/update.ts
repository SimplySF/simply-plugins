...
const messages = Messages.loadMessages('@simplysf/simply-aep', 'simply.aep.at4dx.platform-event-subscription.update');
...
/**
 * Updates an existing AT4DX Platform Event Distributor subscription
 * (`PlatformEvents_Subscription__mdt`) in local DX source and/or a connected org — only the fields
 * given change; everything else, including `Consumer__c`, is preserved from the found record unless
 * explicitly overridden (`Consumer__c` is an ordinary value change here, not the record's key —
 * `DeveloperName`, which this command cannot change, is).
 */
export default class At4dxPlatformEventSubscriptionUpdate extends SfCommand<At4dxPlatformEventSubscriptionUpdateResult> {
  ...
  public static readonly flags = {
    ...
    'event-bus': Flags.string({ summary: messages.getMessage('flags.event-bus.summary') }),
    consumer: Flags.string({ summary: messages.getMessage('flags.consumer.summary') }),
  ...
  public async run(): Promise<At4dxPlatformEventSubscriptionUpdateResult> {
    const { flags } = await this.parse(At4dxPlatformEventSubscriptionUpdate);
