...
import { deployProgressFileFlag, deployRulesFileFlag } from '../../../../../common/deploy/flags.js';

Messages.importMessagesDirectoryFromMetaUrl(import.meta.url);
const messages = Messages.loadMessages('@simplysf/simply-cicd', 'simply.cicd.deploy.happy-soup.validate');

/** Validates deployment configuration files for a happy-soup deployment. */
export default class DeployHappySoupValidate extends SfCommand<void> {
  ...
  public async run(): Promise<void> {
    const { flags } = await this.parse(DeployHappySoupValidate);
