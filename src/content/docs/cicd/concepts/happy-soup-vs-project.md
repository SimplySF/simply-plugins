...
3. `deploy project run-apex-tests` runs against the (already-authenticated) target org directly and runs its Apex tests, since the tests live inside the installed package. An unpackaged project has no installed package to point this at — it runs its tests via `--test-level`/`--test-suite`/`--tests` on the `deploy-unpackaged` stage instead, the same way happy-soup does.
