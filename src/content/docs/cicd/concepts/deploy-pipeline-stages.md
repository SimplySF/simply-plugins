...
Both [`deploy project`](/cicd/reference/deploy-project/) and [`deploy happy-soup`](/cicd/reference/deploy-happy-soup/) expose the same shape of pipeline, one command per stage. Read [Project vs. Happy Soup](/cicd/concepts/happy-soup-vs-project/) first if you haven't — this page assumes you already know which topic you're using.
